...
describe 'Top level App', js: true do
  ...
  it 'will reload the current page if the heart beat stops for 5 seconds' do
    ...
    # we can't use Timecop here because in the next test we actually need to
    # burn the clock for the test to be effective, and we want the tests
    # to work the same in both cases.
    on_client do
      start_time = Time.now
      elapsed_time = 0
      while elapsed_time < 5
        1_000_000.times { elapsed_time = Time.now - start_time }
        # JS needs some IO so timers actually work
  ...
  it 'will not reload the page if paused has been called' do
    on_client { App.history.push('/pray') }
    on_client do
      App.pause_heartbeat do
        start_time = Time.now
        elapsed_time = 0
        while elapsed_time < 5
          1_000_000.times { elapsed_time = Time.now - start_time }
